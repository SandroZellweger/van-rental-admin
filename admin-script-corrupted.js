...
function renderVanCollection(vans) {
    ...
    openSampleSheet() {> {
        // Open a new window with sample sheet informationlly!', 'success');
        const sampleWindow = window.open('', '_blank', 'width=800,height=600');
        sampleWindow.document.write(`h
            <!DOCTYPE html>document.querySelectorAll('.modal');
            <html>.forEach(modal => modal.remove());
            <head>
                <title>Sample Google Sheet Structure</title>
                <style>PricingProfiles();
                    body { font-family: Arial, sans-serif; margin: 20px; }
                    table { border-collapse: collapse; width: 100%; }
                    th, td { border: 1px solid #ddd; padding: 8px; text-align: left; }
                    th { background-color: #f2f2f2; }
                    .note { background: #e3f2fd; padding: 15px; margin: 20px 0; border-radius: 5px; }
                </style>ow = window.open('', '_blank', 'width=800,height=600');
            </head>w.document.write(`
            <body>YPE html>
                ...
                <table>y { font-family: Arial, sans-serif; margin: 20px; }
                    <tr>e { border-collapse: collapse; width: 100%; }
                        <th>VehicleID</th>olid #ddd; padding: 8px; text-align: left; }
                        <th>VehicleName</th>f2f2f2; }
                        <th>CalendarID</th>f2fd; padding: 15px; margin: 20px 0; border-radius: 5px; }
                        <th>VehicleAdress</th>
                        <th>LicencePlate</th>
                        <th>ExtHight</th>
                        <th>ExtLength</th>ructure for Van Data</h1>
                        <th>ExtLarge</th>
                        <th>IntHight</th>> Your Google Sheet should have the following columns in this exact order:
                        <th>IntLength</th>
                        <th>IntLarge</th>
                    </tr>
                    <tr><th>VehicleID</th>
                ...
                <div class="note">ivaro (Losone)</td>
                    <strong>Tips:</strong>ce23@gmail.com</td>
                    <ul><td>Via dei Patrizi 1, 6616 Losone</td>
            ...
            </body>     <td>1.27 / 1.69</td>
            </html> </tr>
        `);     </table>
        sampleWindow.document.close();
    }               <strong>Tips:</strong>
                    <ul>
    // Load sample Swiss van datasure your sheet is publicly viewable or shared with appropriate permissions</li>
    loadRealVanData() { <li>The first row should contain the column headers exactly as shown above</li>
        // Check if real-van-data.js is available separators for dimensions (European format)</li>
        if (typeof getRealVanData === 'function') {your actual Google Calendar IDs for integration</li>
            try {   </ul>
    ...
    openImageSelector(vanId, imageType = 'primary') {
        ...
        console.log('Assigned images:', assignedImages.length);nd. Please ensure real-van-data.js is loaded.', 'error');
        }
        // Create modal
        const modal = document.createElement('div');
        modal.className = 'modal active';gement
        modal.id = 'image-selector-modal';primary') {
        console.log('openImageSelector called with:', { vanId, imageType });
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 900px;">tem.assignedVan);
                <div class="modal-header">tems.filter(item => item.assignedVan && item.assignedVan.toString() === vanId.toString());
                    <h3>Select Image for Van #${vanId}</h3>
                    <button class="modal-close" onclick="this.closest('.modal').remove()">&times;</button>
                </div>ssigned images:', assignedImages.length);
                ...
                <div class="modal-body">
                    ...
                    ss="modal-content" style="max-width: 900px;">
                    <!-- Available Images Tab -->
                    <div class="selector-tab-content active" id="available-tab">
                        ${availableImages.length > 0 ? `"this.closest('.modal').remove()">&times;</button>
                            <div class="image-selector-grid">
                                ${availableImages.map(item => `
                                    <div class="selector-image-item" onclick="adminDashboard.selectImageForVan('${item.id}', ${vanId}, '${imageType}')">
                                        ...
                                        <div class="selector-image-info">"available">Available Images (${availableImages.length})</button>
                                            <div class="image-name">${item.name}</div>mages (${assignedImages.length})</button>
                                            <div class="image-size">${(item.size / 1024 / 1024).toFixed(1)}MB</div>
                                        </div>
                                    </div>
                                `).join('')}b -->
                            </div>lector-tab-content active" id="available-tab">
                        ` : `ilableImages.length > 0 ? `
                            <div class="empty-state">r-grid">
                                <i class="fas fa-images"></i> `
                                <p>No unassigned images available</p>onclick="adminDashboard.selectImageForVan('${item.id}', ${vanId}, '${imageType}')">
                                <button class="btn btn-primary" onclick="document.querySelector('[data-tab=upload]').click()">Upload Images</button>
                            </div>      <div class="selector-image-info">
                        `}                  <div class="image-name">${item.name}</div>
                    </div>                  <div class="image-size">${(item.size / 1024 / 1024).toFixed(1)}MB</div>
                                        </div>
                    <!-- Assigned Images Tab -->
                            ...
                            <div class="image-selector-grid">
                                ${assignedImages.map(item => `
                                    <div class="selector-image-item assigned" onclick="adminDashboard.selectImageForVan('${item.id}', ${vanId}, '${imageType}')">
                                        <img src="${item.dataUrl}" alt="${item.name}">
                                        <div class="selector-image-info">document.querySelector('[data-tab=upload]').click()">Upload Images</button>
                                            <div class="image-name">${item.name}</div>
                                            <div class="image-details">
                                                ${item.category ? `<span class="category-tag">${item.category}</span>` : ''}
                                                ${item.isPrimary ? '<span class="primary-tag">★ Primary</span>' : ''}
                                            </div>
                                        </div>ontent" id="assigned-tab">
                                    </div>ength > 0 ? `
                                `).join('')}e-selector-grid">
                            </div>assignedImages.map(item => `
                        ` : `       <div class="selector-image-item assigned" onclick="adminDashboard.selectImageForVan('${item.id}', ${vanId}, '${imageType}')">
                            <div class="empty-state">tem.dataUrl}" alt="${item.name}">
                                <i class="fas fa-image"></i>-image-info">
                                <p>No images assigned to this van yet</p>m.name}</div>
                            </div>          <div class="image-details">
                        `}                      ${item.category ? `<span class="category-tag">${item.category}</span>` : ''}
                    </div>                      ${item.isPrimary ? '<span class="primary-tag">★ Primary</span>' : ''}
                                            </div>
                    <!-- Upload New Tab -->iv>
                    ...
                    <!-- Upload New Tab -->
                <div class="modal-footer">ab-content" id="upload-tab">
            ...
            </div>              <p>Drop images here or click to browse</p>
        `;                      <input type="file" id="selector-file-input" multiple accept="image/*" style="display: none;">
                                <button class="btn btn-primary" onclick="document.getElementById('selector-file-input').click()">Choose Files</button>
        document.body.appendChild(modal);
    ...
    setupImageSelectorTabs() {dal-footer">
        const tabButtons = document.querySelectorAll('.selector-tab-btn');t('.modal').remove()">Cancel</button>
        const tabContents = document.querySelectorAll('.selector-tab-content');diaManager()">Go to Media Manager</button>
                </div>
        tabButtons.forEach(button => {
            button.addEventListener('click', () => {
                const targetTab = button.getAttribute('data-tab');
                ...
                // Add active class to clicked tab and corresponding content
                button.classList.add('active');orAll('.selector-tab-btn');
                const targetContent = document.getElementById(`${targetTab}-tab`);
                if (targetContent) {
                    targetContent.classList.add('active');
                ...
                tabButtons.forEach(btn => btn.classList.remove('active'));
    setupSelectorUpload() {.forEach(content => content.classList.remove('active'));
        const dropZone = document.getElementById('selector-drop-zone');
        const fileInput = document.getElementById('selector-file-input');ent
                button.classList.add('active');
        if (!dropZone || !fileInput) return;nt.getElementById(`${targetTab}-tab`);
                if (targetContent) {
        // Drag and drop handlers.classList.add('active');
        ...
        dropZone.addEventListener('dragleave', () => {
            dropZone.classList.remove('dragover');selector-drop-zone');
        });st fileInput = document.getElementById('selector-file-input');
    ...
    handleSelectorFileUpload(files) {op', (e) => {
        Array.from(files).forEach(file => {
            if (file.type.startsWith('image/')) {;
                const reader = new FileReader();Transfer.files);
                reader.onload = (e) => {
                    const mediaItem = {
                        id: Date.now() + Math.random(),
                        name: file.name,ge', (e) => {
                        size: file.size,d(e.target.files);
                        dataUrl: e.target.result,
                        uploadDate: new Date().toISOString(),
                        assignedVan: null,
                        category: null,
                    ...
                    if (modal) {te: new Date().toISOString(),
                        ...
                        }, 100);ems.push(mediaItem);
                    }his.saveMediaData();
                };  
                reader.readAsDataURL(file);e images tab
            }       const modal = document.getElementById('image-selector-modal');
        });         if (modal) {
    }                   modal.remove();
                        // Reopen with updated content
    selectImageForVan(imageId, vanId, imageType) {
        ...
        if (!item) return;  const imageType = modal.dataset.imageType;
                            this.openImageSelector(vanId, imageType);
        ...
        if (!item.category) {
            switch(imageType) {
                ...
                default: item.category = 'exterior'; break;g() === imageId.toString());
            }item) return;
        }
        // Update image assignment
        // If this is being set as primary, remove primary from other images of this van
        if (imageType === 'primary') {
            this.mediaItems.forEach(mediaItem => { already set
                if (mediaItem.assignedVan && mediaItem.assignedVan.toString() === vanId.toString() && mediaItem.id !== item.id) {
                    mediaItem.isPrimary = false;
                }ase 'interior': item.category = 'interior'; break;
            }); case 'exterior': item.category = 'exterior'; break;
            item.isPrimary = true;em.category = 'details'; break;
        ...
        // Close modal=== 'primary') {
        const modal = document.getElementById('image-selector-modal');
        if (modal) modal.remove();gnedVan && mediaItem.assignedVan.toString() === vanId.toString() && mediaItem.id !== item.id) {
                    mediaItem.isPrimary = false;
    ...
    openMediaManager() {
        ...
        if (mediaManagerTab) { // Refresh van grid to show new image
            mediaManagerTab.click();
        }/ Close modal
        const modal = document.getElementById('image-selector-modal');
        // Close any open modals);
        const modals = document.querySelectorAll('.modal');
        modals.forEach(modal => modal.remove()); van #${vanId}`, 'success');
    }
    ...
    setupVanImagePlaceholderListeners() {
        ...
        document.querySelectorAll('.van-image-placeholder, .van-thumb-placeholder').forEach(placeholder => {
            placeholder.addEventListener('click', (e) => {
                ...
                this.openImageSelector(vanId, imageType);
            });odals = document.querySelectorAll('.modal');
        });als.forEach(modal => modal.remove());
    }

    getVanImageHTML(vanId, imageType) {erListeners() {
        // Find assigned images for this van       // Add click handlers for van image placeholders
        const assignedImages = this.mediaItems.filter(item =>         document.querySelectorAll('.van-image-placeholder, .van-thumb-placeholder').forEach(placeholder => {
            item.assignedVan && item.assignedVan.toString() === vanId.toString(), (e) => {
        );= parseInt(e.target.getAttribute('data-van-id'));
        
        let targetImage = null;older clicked:', { vanId, imageType });
            this.openImageSelector(vanId, imageType);
        if (imageType === 'primary') {
            // For primary, look for isPrimary=true first, then any image   });
            targetImage = assignedImages.find(img => img.isPrimary) ||    }
                         assignedImages.find(img => img.category === 'exterior') || 
                         assignedImages[0];
        } else {
            // For specific types, look by category
            targetImage = assignedImages.find(img => img.category === imageType);// Global functions that can be called from HTML
        }
        board.loadRealVanData === 'function') {
        if (targetImage) {
            // Return actual image
            const imageSize = imageType === 'primary' ? 'height: 250px;' : 'height: 80px;';     console.error('AdminDashboard not initialized');
    }
}

// Initialize the dashboard when the page loads
// Global adminDashboard variable
let adminDashboard;
